...
TIME_STEP = 3
BATCH_SIZE = 16
HIDDEN_DIM = 9  # output channel number 
NUM_LAYER = 2
DROPOUT = 0.5
...
"""模型结构"""
Size, Hight, Width = 3, 10, 10
Out_feature_num = 2
class myModel(nn.Module):
    def __init__(self):
        super().__init__()
        self.ts_feature = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=TIME_STEP, kernel_size=(Size, Size), padding=0, stride=1),
            nn.ReLU(inplace=True),
            nn.BatchNorm2d(num_features=TIME_STEP),
            # ! 构造伪时序 C0 -> T ：(B0, C0, H0, W0) ==> (B0, T, 1, H0, W0)  
            Reshape(batch_size=BATCH_SIZE, time_step=TIME_STEP, channel=1, hight=Hight, width=Width),
        ...
        self.classfier = nn.Sequential(
            ...
            # ? 中间层神经元数多少合适
            nn.Linear(in_features=Hight*Width*HIDDEN_DIM, out_features=FC_FEATURE),
            nn.ReLU(inplace=True),
            nn.Dropout(DROPOUT),
            nn.Linear(in_features=FC_FEATURE, out_features=Out_feature_num)
            # nn.Softmax(dim=-1)
        )
