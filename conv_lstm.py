...
class myDataset(Dataset):
    def __init__(self):
        super().__init__()
        self.all_data = pd.read_csv(r"E:\Experiments\IntrusionDetection\NSL-KDD_Prep\KDDTrain.txt", header=None)
        self.label = pd.read_csv(r"E:\Experiments\IntrusionDetection\NSL-KDD_Prep\KDDTrain_label_binary.txt", header=None)
    ...
    def __getitem__(self, index):
        ...
        return torch.tensor(a_data), torch.tensor(self.label.loc[index].tolist())
...
class myModel(nn.Module):
    def __init__(self):
        ...
        self.ts_feature = nn.Sequential(
            ...
            ConvLSTM(input_dim=1, hidden_dim=HIDDEN_DIM, kernel_size=(3,3), num_layers=3, batch_first=True)
